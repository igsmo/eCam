...
{
    int ret = 0;
    WRITE_REG_OR_RETURN(BANK_SENSOR, COM7, COM7_SRST); //System reset to factory defaults
    vTaskDelay(10 / portTICK_PERIOD_MS);
    WRITE_REGS_OR_RETURN(ov2640_settings_cif); //Reads a bunch of register settings into the chip
    return ret;
}
...
static int set_window(sensor_t *sensor, ov2640_sensor_mode_t mode, int offset_x, int offset_y, int max_x, int max_y, int w, int h){
    ...
    //WRITE_REG_OR_RETURN(BANK_SENSOR, CLKRC, 0x00); //bypass c.clk and set register manually
    WRITE_REG_OR_RETURN(BANK_DSP, R_DVP_SP, c.pclk);
    //WRITE_REG_OR_RETURN(BANK_DSP, R_DVP_SP, 0x03); //bypass c.pclk and set register manually + Bypass DSP
