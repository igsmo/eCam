...
static const char *TAG = "QR_Posing";
...
static void task_process_handler(void *arg)
{
    ...
    while (true)
    {
        ...
        if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
        {
            ...
            OFrame = frame->buf;
            y = 0;
            // converting image to pure black or white
            while (y < 240)
            {
                x = 0;
                while (x < 240)
                {
                    if((((OFrame[y*240+x]>>11)&(0b11111)) > 24) & (((OFrame[y*240+x]>>5)&(0b111111)) > 50) & (((OFrame[y*240+x])&(0b11111)) > 24))
                    {
            ...
            //checking image from left to right
            found = false;
            k = 0;
            while (y < 240)
            {
                ...
                while (x < 240)
                {
                    //checking if inside area that is already noted
                    inz = false;
                    for(int n = 0; n < 5; n++)
                    {
                        if(l1[n].o)
                    ...
                    //if not inside area already noted looks for Top line of frame
                    if(!inz)
                    {
                        if((PureBOrW[y*240+x] != 0) & (PureBOrW[(y-sTopMargin)*240+x] == 0))
                        {
                            ...
                            //if top line is found tries to build right side line
                            if(found)
                            {
                                i = 1;
                                dy = 1;
                                ...
                                //if right side line is long enough notes corners
                                if(found)
                                {
                                    if(k < 5)
                                    {
            ...
            //going through image right to left
            y = 1;
            while (y < 240)
            {
                x = 239;
            ...
            best.dx = 0;
            best.f = false;
            //sends x position over uart, sends #as start character and sends 500 if nothing was found
            for(int z = 0; z < 5; z++)
            {
                if(l1[z].o)
                {
                    if((l1[z].f ? l1[z].x0 - l1[z].x1 : l1[z].x1 - l1[z].x0) > 130)
                    {
                        esp_image_scanner_t *esp_scn = esp_code_scanner_create();
                        esp_code_scanner_config_t config = {ESP_CODE_SCANNER_MODE_FAST, ESP_CODE_SCANNER_IMAGE_RGB565, 240, 240};
                        esp_code_scanner_set_config(esp_scn, config);
                        int decoded_num = esp_code_scanner_scan_image(esp_scn, PureBOrW);

                        if(decoded_num){
                            esp_code_scanner_symbol_t result = esp_code_scanner_result(esp_scn);
                            ESP_LOGI(TAG, "~%s", result.data);
                    ...
                    //, l1[z].f ? l1[z].y0 - l1[z].y1 : l1[z].y1 - l1[z].y0);
            ...
            if(!best.o)
            {
                ESP_LOGI(TAG, "#%d;%d", 500, 5);
            }
            else
            {
                ESP_LOGI(TAG, "#%d;%d", (best.x1 + best.x0) / 2, best.f ? best.x0 - best.x1 : best.x1 - best.x0);
            }
